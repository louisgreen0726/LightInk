...
- **统一书库**：冷启动进入「我的书」封面墙；点「编辑」进入 Markdown 编辑器，编辑器内「阅读/书架」回到封面墙。集中管理本地书籍与 OPDS 1.x 作品，支持目录、搜索、分页、封面以及 Basic/Bearer 鉴权
...
应用冷启动进入阅读器书架（「我的书」封面墙）。书架上的「编辑」进入 Markdown 编辑器（菜单栏与多标签仍可用）；编辑器里的「阅读/书架」回到封面墙。往返只走这两处带文字的入口，不再用「文件 → 书库」把书架切去编辑器。可导入本地书籍，或添加 OPDS 1.x Atom/XML 目录。OPDS 源默认要求 HTTPS；HTTP/LAN 源必须在添加时明确勾选允许。鉴权支持 Basic 和 Bearer，不支持 OAuth、Cookie 登录或 OPDS 2.0。
