...
fn write_blob(app_data_dir: &Path, hash: &str, bytes: &[u8]) -> Result<PathBuf, String> {
    ...
    temporary
        .persist(&path)
        .map_err(|error| format!("无法提交文档正文: {}", error.error))?;
    Ok(path)
}
...
fn copy_asset(
    ...
    if !target.is_file() {
        ...
        temporary
            .persist(&target)
            .map_err(|error| format!("无法提交资源: {}", error.error))?;
    }
    ...
    connection
        .execute(
            "INSERT INTO managed_assets(hash,relative_path,size,media_type,created_at,updated_at)
             VALUES (?1,?2,?3,?4,?5,?5)
             ON CONFLICT(hash) DO UPDATE SET updated_at=?5",
            params![
                hash,
                stored_relative,
                bytes.len() as i64,
        ...
        .map_err(|error| format!("无法记录受管资源: {error}"))?;
    Ok(ManagedAsset {
        hash,
...
fn join_document_at(
    ...
    if let Some(existing_id) = connection
        .query_row(
            "SELECT id FROM managed_documents WHERE content_hash=?1",
            params![content_hash],
            |row| row.get::<_, String>(0),
        )
        .optional()
        .map_err(|error| format!("无法查询受管文档: {error}"))?
    {
        let (document, content) = read_document_at(connection, app_data_dir, &existing_id)?;
        return Ok(JoinDocumentResult {
            document,
            content,
    ...
    for reference in asset_references(&content) {
        ...
        match copy_asset(connection, app_data_dir, &id, &canonical_source, &reference) {
            Ok(asset) => copied_assets.push(asset),
    ...
    write_blob(app_data_dir, &blob_hash, content.as_bytes())?;
    ...
    transaction
        .commit()
        .map_err(|error| format!("无法提交受管文档: {error}"))?;
...
pub fn managed_document_create_version(
    ...
    write_blob(&app_data, &hash, content.as_bytes())?;
    let now = library::now_ms();
    let version_id = Uuid::new_v4().to_string();
    let transaction = connection
        .transaction()
        .map_err(|error| format!("无法开启版本事务: {error}"))?;
    transaction
        .execute(
    ...
    transaction.execute("INSERT INTO document_versions(id,document_id,blob_hash,size,device_id,created_at,is_current) VALUES (?1,?2,?3,?4,?5,?6,1)", params![version_id, document.id, hash, content.len() as i64, device_id, now]).map_err(|error| format!("无法写入文档版本: {error}"))?;
    transaction
        .execute(
    ...
    transaction
        .commit()
        .map_err(|error| format!("无法提交文档版本: {error}"))?;
    Ok(DocumentVersion {
        id: version_id,
...
pub fn managed_document_save_draft(
    ...
    let now = library::now_ms();
    let id = Uuid::new_v4().to_string();
    connection.execute("INSERT INTO document_drafts(id,document_id,blob_hash,title,device_id,created_at,updated_at) VALUES (?1,?2,?3,?4,?5,?6,?6)", params![id, document_id, hash, title, device_id, now]).map_err(|error| format!("无法保存文档草稿: {error}"))?;
    Ok(DocumentDraft {
        id,
        document_id,
        blob_hash: hash,
